...
class Trader:
    ...
    TRIG_DN_B2 =  -90          # buy (used to be -70)
    STOP_LOSS_B2 = -140        # new testing stoploss

    STEP_B2    =   30
    EXIT_BAND  =   20          # flatten when |spread–theta| < 20
    ...
    def trade_basket2(self, depths: Dict[str, OrderDepth], pos: int) -> List[Order]:
        ...
        # BUY (under priced)
        if spread < self.TRIG_DN_B2 and pos < limit:
            qty = min(self.STEP_B2, limit - pos,
                      abs(depth.sell_orders[self.best_ask(depth)]))
            if qty > 0:
                orders.append(Order("PICNIC_BASKET2", self.best_ask(depth),  qty))
        
        # hard stop‑loss (!TODO: need to tweak for test, nto triggerning rn??)
