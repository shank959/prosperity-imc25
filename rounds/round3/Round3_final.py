...
class Trader:
    ...
    def rfr_orders(self, orderDep: OrderDepth, fairVal: int, width: int, position: int, positionLimit: int) -> List[Order]:
        ...
        baaf = min([price for price in orderDep.sell_orders.keys() if price > fairVal + 1])\
            if len([price for price in orderDep.sell_orders.keys() if price > fairVal + 1]) != 0\
            else fairVal + 2
        bbbf = max([price for price in orderDep.buy_orders.keys() if price < fairVal - 1])\
            if len([price for price in orderDep.buy_orders.keys() if price < fairVal - 1]) != 0\
            else fairVal - 2
    ...
    def cross_pic_order(self, order_depth: OrderDepth, position: int, position_limit: int):
        ...
        pic1_ba = min(pic1_order_depth.sell_orders.keys()) if pic1_order_depth.sell_orders else None
        pic2_ba = min(pic2_order_depth.sell_orders.keys()) if pic2_order_depth.sell_orders else None
        djembe_ba = min(djembes_order_depth.sell_orders.keys()) if djembes_order_depth.sell_orders else None

        pic1_bb = max(pic1_order_depth.buy_orders.keys()) if pic1_order_depth.buy_orders else None
        pic2_bb = max(pic2_order_depth.buy_orders.keys()) if pic2_order_depth.buy_orders else None
        djembe_bb = max(djembes_order_depth.buy_orders.keys()) if djembes_order_depth.buy_orders else None
    ...
    def volcanic_rock_iv(self, K, timestamp, order_depth: OrderDepth):
        # volcanic rock and underlying
        # These variables should be provided in your actual context.
        # Here are example placeholder values:
        ...
        # Get the best ask and bid prices for the underlying asset
    ...
    def volcanic_rock_voucher_order(self, order_depth: OrderDepth, position: Dict[str, int], lim: int, v_lim: int, timestamp: int):
        orders = []
        volcanic_rock_order_depth = order_depth["VOLCANIC_ROCK"]
        ...
        threshold = 0.1 # arbitrary place holder

        # print(iv_s, m_s)

        print(coeffs[0] * m_s[0]**2 + coeffs[1] * m_s[0] + coeffs[2] - iv_s[0])
        ...
        for i, K in enumerate(strikes):
            ...
            if position[f'VOLCANIC_ROCK_VOUCHER_{K}'] > 0:
                best_bid = max(order_depth[f'VOLCANIC_ROCK_VOUCHER_{K}'].buy_orders.keys()) if order_depth[f'VOLCANIC_ROCK_VOUCHER_{K}'].buy_orders else 0
                pos_sell_amt = abs(order_depth[f'VOLCANIC_ROCK_VOUCHER_{K}'].buy_orders[best_bid]) if best_bid != 0 else 0
                pos_sell_amt = min(pos_sell_amt, position[f'VOLCANIC_ROCK_VOUCHER_{K}'])
                if pos_sell_amt > 0:
                    orders.append(Order(f'VOLCANIC_ROCK_VOUCHER_{K}', best_bid, round(-pos_sell_amt)))

            if position[f'VOLCANIC_ROCK_VOUCHER_{K}'] < 0:
                best_ask = min(order_depth[f'VOLCANIC_ROCK_VOUCHER_{K}'].sell_orders.keys()) if order_depth[f'VOLCANIC_ROCK_VOUCHER_{K}'].sell_orders else 0
                pos_buy_amt = abs(order_depth[f'VOLCANIC_ROCK_VOUCHER_{K}'].sell_orders[best_ask]) if best_ask != 0 else 0
                pos_buy_amt = min(pos_buy_amt, position[f'VOLCANIC_ROCK_VOUCHER_{K}'])
                if pos_buy_amt > 0:
                    orders.append(Order(f'VOLCANIC_ROCK_VOUCHER_{K}', best_ask, round(pos_buy_amt)))

        if position["VOLCANIC_ROCK"] > 0:
            best_bid = max(order_depth['VOLCANIC_ROCK'].buy_orders.keys()) if order_depth['VOLCANIC_ROCK'].buy_orders else 0
            pos_sell_amt = abs(order_depth['VOLCANIC_ROCK'].buy_orders[best_bid]) if best_bid != 0 else 0
            pos_sell_amt = min(pos_sell_amt, position["VOLCANIC_ROCK"])
            if pos_sell_amt > 0:
                orders.append(Order('VOLCANIC_ROCK', best_bid, round(-pos_sell_amt)))

        if position["VOLCANIC_ROCK"] < 0:
            best_ask = min(order_depth['VOLCANIC_ROCK'].sell_orders.keys()) if order_depth['VOLCANIC_ROCK'].sell_orders else 0
            pos_buy_amt = abs(order_depth['VOLCANIC_ROCK'].sell_orders[best_ask]) if best_ask != 0 else 0
            pos_buy_amt = min(pos_buy_amt, position["VOLCANIC_ROCK"])
            if pos_buy_amt > 0:
                orders.append(Order('VOLCANIC_ROCK', best_ask, round(pos_buy_amt)))
    ...
    def run(self, state: TradingState):
        ...
        if "VOLCANIC_ROCK" in state.order_depths:
            ...
            for order in volcanic_rock_orders:
