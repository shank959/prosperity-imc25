...
class Trader:
    ...
    #         ) -> List[Order]:
    #     """
    #     Flatten any residual position in `product` by walking the book across price levels.
    #     """
    #     orders: List[Order] = []
    ...
    #     # If we're long, sell into bids (highest price first)
    #     if pos > 0:
    #         remaining = pos
    #         for price, volume in sorted(
    #             order_depth[product].buy_orders.items(),
    #             key=lambda kv: kv[0],
    #             reverse=True
    #         ):
    #             if remaining <= 0:
    ...
    #             remaining -= qty
    #     # If we're short, buy from asks (lowest price first)
    #     else:
    #         remaining = -pos
    #         for price, volume in sorted(
    #             order_depth[product].sell_orders.items(),
    #             key=lambda kv: kv[0]
    #         ):
    #             if remaining <= 0:
    #                 break
