...
const PRODUCTS: [&str; 3] = ["EMERALDS", "TOMATOES", "ASH_COATED_OSMIUM"];
...
// OSMIUM calibrated constants (from calibration/osmium/scripts/)
const OSMIUM_TRADE_ACTIVE_PROB: f64 = 1265.0 / 30_000.0;
const OSMIUM_SECOND_TRADE_PROB: f64 = 13.0 / 1252.0;
const OSMIUM_TRADE_BUY_PROB: f64 = 640.0 / 1264.0;
// Book structure (from hold-1-unit FV extraction, live Round 1 day 0):
// Inner and outer bots are independent per side.
// Inner bot: round(FV)±8, vol U(10,15).
// Outer bot: floor(FV)-10 bid / ceil(FV)+10 ask, vol U(20,30).
const OSMIUM_INNER_BID_PRESENT_PROB: f64 = 0.7551;
const OSMIUM_INNER_ASK_PRESENT_PROB: f64 = 0.8082;
const OSMIUM_OUTER_BID_PRESENT_PROB: f64 = 0.7876;
const OSMIUM_OUTER_ASK_PRESENT_PROB: f64 = 0.7910;
// Spike: depth-triggered — only fires when outer bot is present on at least one side (9.125%).
// When no outer bot on either side, spike rate = 0.0%.
const OSMIUM_SPIKE_PROB_GIVEN_OUTER: f64 = 73.0 / 800.0; // 9.125%
const OSMIUM_SPIKE_BID_FRAC: f64 = 1244.0 / 2411.0; // 51.6% of spikes are bid-side
const OSMIUM_SPIKE_BID_CROSSING_FRAC: f64 = 391.0 / 1245.0; // 31.4% of bid spikes cross FV
const OSMIUM_SPIKE_ASK_CROSSING_FRAC: f64 = 412.0 / 1167.0; // 35.3% of ask spikes cross FV
const OSMIUM_FV_SIGMA: f64 = 0.3117; // live Round 1 tick sigma (trained: 0.3223, live: 0.3117)
        ...
        if config.trade_mode == TradeMode::ReplayTimes {
            for day in DAYS {
                ...
                for product in PRODUCTS {
                    // OSMIUM always uses simulated trade counts — no round0 data file
...
fn generate_day(day: i32, config: &Config, replay: &ReplayData) -> Result<DayOutput> {
    ...
    let osmium_fair_state = simulate_osmium_fair(day, config.ticks_per_day, &mut rng);
    ...
    let osmium_trade_counts = trade_counts_for("ASH_COATED_OSMIUM", day, config, replay, &mut rng)?;
    ...
    for tick in 0..config.ticks_per_day {
        ...
        let osmium_book = make_osmium_book(osmium_fair_state[tick], &mut rng);
        ...
        price_rows.push(book_to_price_row(day, timestamp, "ASH_COATED_OSMIUM", &osmium_book));
        ...
        for _ in 0..osmium_trade_counts[tick] {
            trade_rows.extend(sample_trade_rows(timestamp, "ASH_COATED_OSMIUM", &osmium_book, &mut rng));
        }
    }
...
fn run_backtest_session(
    ...
    for day in [session_day] {
        ...
        for tick in 0..config.ticks_per_day {
            ...
            let order_depths = HashMap::from([
                ...
                ("ASH_COATED_OSMIUM".to_string(), book_to_worker_depth(&osmium_book)),
            ]);
            ...
            let mut live_books = HashMap::from([
                ...
                ("ASH_COATED_OSMIUM".to_string(), book_to_sim_book(&osmium_book)),
            ]);
            ...
            for (product, count) in [("EMERALDS", emerald_trade_counts[tick]), ("TOMATOES", tomato_trade_counts[tick]), ("ASH_COATED_OSMIUM", osmium_trade_counts[tick])] {
                let product_key = product.to_string();
                let book = live_books
                    .get_mut(&product_key)
            ...
            if capture_outputs {
                for product in PRODUCTS {
                    ...
                    let fair = match product {
                        "EMERALDS" => 10_000.0,
                        "TOMATOES" => tomato_latent_state[tick].fair,
                        "ASH_COATED_OSMIUM" => osmium_fair_state[tick],
                        _ => 0.0,
                    };
            ...
            let osmium_ledger = ledgers.get("ASH_COATED_OSMIUM").context("missing osmium ledger for fit")?;
            ...
            let osmium_mtm =
                osmium_ledger.cash + osmium_ledger.position as f64 * osmium_fair_state[tick];
            ...
            total_fit.update(session_x, emerald_mtm + tomato_mtm + osmium_mtm);
            day_emerald_fit.update(day_x, emerald_mtm);
            day_tomato_fit.update(day_x, tomato_mtm);
            day_osmium_fit.update(day_x, osmium_mtm);
            day_total_fit.update(day_x, emerald_mtm + tomato_mtm + osmium_mtm);
            global_step += 1;
            day_step += 1;
        ...
        let tomato_fair = tomato_latent_state
            ...
            .unwrap_or(5_000.0);
        let osmium_fair_final = osmium_fair_state.last().copied().unwrap_or(10_000.0);
        ...
        let osmium_ledger_final = ledgers.get("ASH_COATED_OSMIUM").context("missing osmium ledger")?;
        ...
        let osmium_pnl = osmium_ledger_final.cash + osmium_ledger_final.position as f64 * osmium_fair_final;
        ...
        osmium_cash_total += osmium_ledger_final.cash;
        emerald_final_position = emerald_ledger.position;
        tomato_final_position = tomato_ledger.position;
        osmium_final_position = osmium_ledger_final.position;

        run_summaries.push(RunSummary {
            session_id,
            day,
            total_pnl: emerald_pnl + tomato_pnl + osmium_pnl,
            emerald_pnl,
            tomato_pnl,
            osmium_pnl,
            ...
            osmium_slope_per_step: day_osmium_fit.slope_per_step(),
            osmium_r2: day_osmium_fit.r_squared(),
        });
    ...
    let summary = SessionSummary {
        ...
        osmium_slope_per_step: osmium_fit.slope_per_step(),
        osmium_r2: osmium_fit.r_squared(),
    };
...
fn trade_counts_for(
    ...
    // OSMIUM always uses simulated trade counts
    if product == "ASH_COATED_OSMIUM" {
        return Ok(simulate_trade_counts(product, config.ticks_per_day, rng));
    }
...
fn simulate_trade_counts(product: &str, ticks: usize, rng: &mut ChaCha8Rng) -> Vec<usize> {
    let (base_prob, second_trade_prob) = match product {
        ...
        _ => (TOMATOES_TRADE_ACTIVE_PROB, 0.0),
    };
...
fn book_to_price_row(day: i32, timestamp: i32, product: &str, book: &Book) -> PriceRow {
    ...
    let mid_price = match (book.bids.first(), book.asks.first()) {
        (Some(b), Some(a)) => (b.0 as f64 + a.0 as f64) / 2.0,
        (Some(b), None) => b.0 as f64,
        (None, Some(a)) => a.0 as f64,
        (None, None) => 0.0,
    };
...
/// Simulate OSMIUM fair value as a slow random walk (σ=0.3223/tick).
/// Day-specific starting values calibrated from EMA FV daily means.
fn simulate_osmium_fair(day: i32, ticks: usize, rng: &mut ChaCha8Rng) -> Vec<f64> {
    let start = match day {
        -2 => 9998.0,
        -1 => 10001.0,
        0 => 10002.0,
    ...
    let mut states = vec![0.0_f64; ticks];
    states[0] = start;
    for i in 1..ticks {
        states[i] = states[i - 1] + OSMIUM_FV_SIGMA * sample_standard_normal(rng);
    }
    states
}

/// Build an OSMIUM order book for a single tick.
///
/// Inner and outer bots are independent per side (from hold-1-unit FV extraction):
///   - Inner bot: round(FV)±8, vol U(10,15). Present independently on bid/ask.
///   - Outer bot: floor(FV)-10 bid / ceil(FV)+10 ask, vol U(20,30). Present independently.
///   - Spike bot (Bot 3): depth-triggered — only fires when outer bot present on ≥1 side.
///     Replaces whichever side it appears on as L1; inner (if present) drops to L2.
fn make_osmium_book(fair: f64, rng: &mut ChaCha8Rng) -> Book {
    let rfv = fair.round() as i32;
    let outer_bid = fair.floor() as i32 - 10;
    let outer_ask = fair.ceil() as i32 + 10;

    // Independent presence per bot per side
    let inner_bid = rng.gen_bool(OSMIUM_INNER_BID_PRESENT_PROB);
    let inner_ask = rng.gen_bool(OSMIUM_INNER_ASK_PRESENT_PROB);
    let outer_bid_present = rng.gen_bool(OSMIUM_OUTER_BID_PRESENT_PROB);
    let outer_ask_present = rng.gen_bool(OSMIUM_OUTER_ASK_PRESENT_PROB);

    // Spike: depth-triggered — only fires when outer bot is present on at least one side
    let outer_any = outer_bid_present || outer_ask_present;
    let (spike_bid, spike_ask) = if outer_any && rng.gen_bool(OSMIUM_SPIKE_PROB_GIVEN_OUTER) {
        if rng.gen_bool(OSMIUM_SPIKE_BID_FRAC) {
            (Some(osmium_spike_bid_quote(fair, rng)), None)
        } else {
            (None, Some(osmium_spike_ask_quote(fair, rng)))
        }
    } else {
        (None, None)
    };
    ...
    // Build bid side
    if outer_bid_present {
        bids.push((outer_bid, rng.gen_range(20..=30)));
    }
    if inner_bid {
        let inner_vol = rng.gen_range(10..=15);
        if let Some((spike_price, spike_vol)) = spike_bid {
            // Spike takes L1, inner drops to L2
            bids.push((spike_price, spike_vol));
            bids.push((rfv - 8, inner_vol));
        } else {
            bids.push((rfv - 8, inner_vol));
        }
    } else if let Some((spike_price, spike_vol)) = spike_bid {
        // Spike is L1, no inner behind it
        bids.push((spike_price, spike_vol));
    }
    bids.sort_by(|a, b| b.0.cmp(&a.0)); // descending (L1 = highest bid)

    // Build ask side
    if outer_ask_present {
        asks.push((outer_ask, rng.gen_range(20..=30)));
    }
    if inner_ask {
        let inner_vol = rng.gen_range(10..=15);
        if let Some((spike_price, spike_vol)) = spike_ask {
            asks.push((spike_price, spike_vol));
            asks.push((rfv + 8, inner_vol));
        } else {
            asks.push((rfv + 8, inner_vol));
        }
    } else if let Some((spike_price, spike_vol)) = spike_ask {
        asks.push((spike_price, spike_vol));
    }
    asks.sort_by(|a, b| a.0.cmp(&b.0)); // ascending (L1 = lowest ask)

    Book { bids, asks }
}

/// Generate a spike bid quote: price and volume.
/// Passive (below FV): vol U(2,5). Crossing (at/above FV): vol U(4,10).
fn osmium_spike_bid_quote(fair: f64, rng: &mut ChaCha8Rng) -> (i32, i32) {
    let fv = fair.round() as i32;
    if rng.gen_bool(OSMIUM_SPIKE_BID_CROSSING_FRAC) {
        // Crossing: bid at or above FV (+1: 48%, +2: 45%, 0: 5%, +3: 2%)
        let r: f64 = rng.gen();
        let offset = if r < 0.484 { 1 } else if r < 0.935 { 2 } else if r < 0.983 { 0 } else { 3 };
        (fv + offset, rng.gen_range(4..=10))
    } else {
        // Passive: bid below FV (-3: 45%, -2: 44%, -4: 7%, -1: 4%)
        let r: f64 = rng.gen();
        let offset = if r < 0.449 { -3 } else if r < 0.892 { -2 } else if r < 0.960 { -4 } else { -1 };
        (fv + offset, rng.gen_range(2..=5))
    }
}

/// Generate a spike ask quote: price and volume.
/// Passive (above FV): vol U(2,5). Crossing (at/below FV): vol U(4,10).
fn osmium_spike_ask_quote(fair: f64, rng: &mut ChaCha8Rng) -> (i32, i32) {
    let fv = fair.round() as i32;
    if rng.gen_bool(OSMIUM_SPIKE_ASK_CROSSING_FRAC) {
        // Crossing: ask at or below FV (-2: 45%, -3: 44%, 0: 7%, -1: 4%)
        let r: f64 = rng.gen();
        let offset = if r < 0.451 { -2 } else if r < 0.888 { -3 } else if r < 0.957 { 0 } else { -1 };
        (fv + offset, rng.gen_range(4..=10))
    } else {
        // Passive: ask above FV (+1: 47%, +2: 45%, +3: 8%)
        let r: f64 = rng.gen();
        let offset = if r < 0.484 { 1 } else if r < 0.939 { 2 } else { 3 };
        (fv + offset, rng.gen_range(2..=5))
    }
}
...
fn sample_trade_quantity_by_side(
    ...
    let (values, weights): (&[i32], &[u32]) = match (product, market_buy) {
        ...
        // OSMIUM: roughly uniform over 2–9 (analyze_trades.py: range 2-10, near-uniform)
        ("ASH_COATED_OSMIUM", true) => (&[2, 3, 4, 5, 6, 7, 8, 9], &[1, 1, 1, 1, 1, 1, 1, 1]),
        ("ASH_COATED_OSMIUM", false) => (&[2, 3, 4, 5, 6, 7, 8, 9], &[1, 1, 1, 1, 1, 1, 1, 1]),
        _ => (&[1], &[1]),
    };
