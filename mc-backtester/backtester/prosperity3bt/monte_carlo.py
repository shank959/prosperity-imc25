        ...
        parsed.append(
            {
                ...
                "osmiumSlopePerStep": float(row.get("osmium_slope_per_step", 0.0) or 0.0),
                "osmiumR2": float(row.get("osmium_r2", 0.0) or 0.0),
            }
        )
    return parsed
...
def load_sample_session(session_dir: Path) -> dict[str, Any]:
    ...
    for day_index, day in enumerate(day_files):
        ...
        for row in trace_rows:
            ...
            ts = DAY_OFFSETS.get(day, day_index * 1_000_000) + int(row["timestamp"])
    ...
    timestamps = next(iter(products.values()))["timestamps"]
    total_pnl = []
...
def build_dashboard(output_dir: Path, algorithm: Path, sessions: int, config: dict[str, Any]) -> dict[str, Any]:
    ...
    osmium_profitability = [row["osmiumSlopePerStep"] for row in run_rows]
    osmium_stability = [row["osmiumR2"] for row in run_rows]
    ...
    session_osmium_profitability = [row["osmiumSlopePerStep"] for row in session_rows]
    session_osmium_stability = [row["osmiumR2"] for row in session_rows]
    ...
    osmium_normal_fit = normal_fit(osmium)
    ...
    return {
        ...
        "overall": {
            ...
            "osmiumPnl": summarize_distribution(osmium),
        ...
        "trendFits": {
            ...
            "ASH_COATED_OSMIUM": {
                "profitability": summarize_distribution(osmium_profitability),
                "stability": summarize_distribution(osmium_stability),
            },
        },
        "aggregateTrendFits": {
            "TOTAL": {
            ...
            "ASH_COATED_OSMIUM": {
                "profitability": summarize_distribution(session_osmium_profitability),
                "stability": summarize_distribution(session_osmium_stability),
            },
        },
        "normalFits": {
        ...
        "generatorModel": {
            ...
            "ASH_COATED_OSMIUM": {
                "name": "Latent Fair Random Walk",
                "formula": "x_{t+1} = x_t + ε_t, σ=0.3117/tick",
                "notes": [
                    "Inner bot: round(FV)±8 (present 75.5% bid / 80.8% ask independently)",
                    "Outer bot: floor(FV)-10 / ceil(FV)+10 (present 78.8% / 79.1% independently)",
                    "Spike quotes: 9.1% of ticks when outer bot present, 0% otherwise (depth-triggered)",
                ],
            },
        },
        "products": {
            ...
            "ASH_COATED_OSMIUM": {
                "pnl": summarize_distribution(osmium),
                "finalPosition": summarize_distribution([float(value) for value in osmium_pos]),
        ...
        "histograms": {
            ...
            "osmiumProfitability": histogram(osmium_profitability),
            "osmiumStability": histogram(osmium_stability),
        },
